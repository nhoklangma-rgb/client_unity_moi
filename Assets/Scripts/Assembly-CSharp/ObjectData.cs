...
public class ObjectData
{
	...
	private static List<string> _delKeysCache = new List<string>(32);
	...
	public static void checkDelHash(MyHashTable hash, int time, bool isTrue)
	{
		...
		if (isTrue)
		{
			while (enumerator.MoveNext())
			{
				...
				if (mainImage != null && mainImage.img != null && mainImage.img.image != null && mainImage.img.image.texture != null)
				{
					UnityEngine.Object.Destroy(mainImage.img.image.texture, 0.5f);
				}
			}
			hash.clear();
			return;
		...
		for (int i = 0; i < _delKeysCache.Count; i++)
		{
			string key = _delKeysCache[i];
			MainImage mainImage = (MainImage)hash.get(key);
			if (mainImage != null && mainImage.img != null && mainImage.img.image != null && mainImage.img.image.texture != null)
			{
				UnityEngine.Object.Destroy(mainImage.img.image.texture, 0.5f);
			}
	...
	public static void checkDelHash_Data(MyHashTable hash, int time, bool isTrue)
	{
		...
		if (isTrue)
		{
			while (enumerator.MoveNext())
			{
				...
				if (effectData != null && effectData.img != null && effectData.img.image != null && effectData.img.image.texture != null)
				{
					UnityEngine.Object.Destroy(effectData.img.image.texture, 0.5f);
				}
			}
			hash.clear();
			return;
		...
		{
			string key = _delKeysCache[i];
			EffectData effectData = (EffectData)hash.get(key);
			if (effectData != null && effectData.img != null && effectData.img.image != null && effectData.img.image.texture != null)
			{
				UnityEngine.Object.Destroy(effectData.img.image.texture, 0.5f);
			}
