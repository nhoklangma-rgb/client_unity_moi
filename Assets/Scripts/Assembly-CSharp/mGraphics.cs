...
public class mGraphics
{
	...
	private void cache(string key, Texture2D value)
	{
		...
		{
			// Destroy all cached textures to free VRAM before clearing
	...
	public void __drawRegion(Image image, int x0, int y0, int w, int h, int transform, float x, float y, int anchor)
	{
		...
		Graphics.DrawTexture(new Rect(x + num3 + num14 + (float)num16, y + num4 + (float)num17 + num15, num * num5, num2 * (float)num7), image.texture, new Rect(((float)x0 + num3 + num6) / (float)image.w, ((float)image.h - num2 - ((float)y0 + num4)) / (float)image.h, num / (float)image.w, num2 / (float)image.h), 0, 0, 0, 0);
		if (transform == 5 || transform == 6 || transform == 4 || transform == 7)
		{
	...
	public void _drawRegion(Image image, float x0, float y0, int w, int h, int transform, int x, int y, int anchor)
	{
		...
		Graphics.DrawTexture(new Rect((float)x + num3 + num14 + (float)num16, (float)y + num4 + (float)num17 + num15, num * num7, num2 * (float)num9), image.texture, new Rect((x0 + num3 + num8) / (float)image.w, ((float)image.h - num2 - (y0 + num4)) / (float)image.h, num / (float)image.w, num2 / (float)image.h), 0, 0, 0, 0);
		if (transform == 5 || transform == 6 || transform == 4 || transform == 7)
		{
			GUI.matrix = matrixBackup;
		}
	...
	public void fillRecAlpla(int x, int y, int w, int h, int color)
	{
		...
		drawRecAlpa(0, y, x, GameCanvas.loadmap.mapW * 24 - y, color);
