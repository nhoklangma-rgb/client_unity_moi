...
public class mFont
{
	...
	public mVector splitFontVector(string src, int lineWidth)
	{
		...
		for (int i = 0; i < src.Length; i++)
		{
			...
			text += src[i];
			currentWidth += getCharWidth(src[i]);
	...
	{
		if (cachedZoomLevel != mGraphics.zoomLevel)
		{
			charWidths.Clear();
			cachedZoomLevel = mGraphics.zoomLevel;
		}
		int width;
		if (!charWidths.TryGetValue(c, out width))
		{
			width = getWidthExactOf(c.ToString());
			charWidths[c] = width;
		}
		return width;
	}
	...
	public int getWidthExactOf(string s)
	{
		try
		{
			if (cachedStyle == null)
			{
				...
				cachedStyle.font = myFont;
			}
			cachedStyle.fontSize = sizef * mGraphics.zoomLevel;
			...
			return (int)cachedStyle.CalcSize(cachedContent).x / mGraphics.zoomLevel;
		}
		catch (Exception ex)
		{
	...
	public int getHeight()
	{
		...
		try
		{
			...
			cachedContent.text = "Adg";
			height = (int)cachedStyle.CalcSize(cachedContent).y + 2;
		}
		catch (Exception ex)
		{
	...
	public void _drawString(mGraphics g, string st, int x0, int y0, int align)
	{
		...
		case 0:
			num = x0;
			num2 = y0;
			cachedStyle.alignment = TextAnchor.UpperLeft;
			break;
		case 1:
			num = x0 - MotherCanvas.w;
			num2 = y0;
			cachedStyle.alignment = TextAnchor.UpperRight;
			break;
		case 2:
		case 3:
			num = x0 - MotherCanvas.w / 2;
			num2 = y0;
			cachedStyle.alignment = TextAnchor.UpperCenter;
			break;
		}
		cachedStyle.normal.textColor = color1;
		g.drawString(st, (int)num, (int)num2, cachedStyle);
	}
