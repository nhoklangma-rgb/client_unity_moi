...
public class DataSkillEff
{
	...
	public DataSkillEff(short ideff, int x, int y, int time)
	{
		idEff = ideff;
		this.x = x;
		this.y = y;
			...
			for (int j = 0; j < num4; j++)
			{
				...
				for (int k = 0; k < b; k++)
				{
					...
					if (flag)
					{
						...
						if (changeFlip)
						{
							partFrame.flip = (sbyte)((partFrame.flip == 0) ? 1 : 0);
						}
					}
					if (partFrame.onTop == 0)
					{
